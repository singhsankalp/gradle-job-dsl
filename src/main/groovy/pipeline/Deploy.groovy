      ...
      shell('''
          case $action in
           "Deploy")
              ...
              QUERY="chef_environment:${this.environment} AND run_list:recipe\\[${this.app}\\:\\:${this.deployconfig.recipe}\\]" ansible-playbook -i inventory/chef deployment.yml -e "concurrency=${concurrencyPercentage} max_fail_percentage=${this.deployconfig.maxFailPercentage} haproxy_query=${this.deployconfig.haproxyQuery} haproxy_backend=${this.deployconfig.haproxyBackend} action=${action}"
              ;;
           "restarted")
              QUERY="chef_environment:${this.environment} AND run_list:recipe\\[${this.app}\\:\\:${this.deployconfig.recipe}\\]" ansible-playbook -i inventory/chef deployment.yml -e "concurrency=${concurrencyPercentage} max_fail_percentage=${this.deployconfig.maxFailPercentage} haproxy_query=${this.deployconfig.haproxyQuery} haproxy_backend=${this.deployconfig.haproxyBackend} action=${action}"
              ;;
